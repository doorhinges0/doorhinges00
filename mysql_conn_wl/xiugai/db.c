...
//		OSAL_trace( eOSAL, eError, "mysql_init fail");
		fprintf (stderr, "mysql_init fail  :%s\n", strerror(errno));
		return RET_ERR;
	}
    ...
    {
//		OSAL_trace( eOSAL, eError, "db_create_conn:connect to database fail(%s:%s@%s:%d)! error: %s", 
//				gdb_config.user, gdb_config.pwd, gdb_config.host, gdb_config.port, mysql_error(*conn));
		fprintf (stderr, "db_create_conn:connect to database fail(%s:%s@%s:%d)! error: %s",gdb_config.user, gdb_config.pwd, gdb_config.host, gdb_config.port, strerror(errno));
...
//						OSAL_trace( eOSAL, eError, "mysql_ping failed!");
							fprintf (stderr, "mysql_ping fail  :%s\n", strerror(errno));
					}
				}
