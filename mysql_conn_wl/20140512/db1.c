...
//			LOG( LOG_ERROR,"db_create_conn:connect to database fail(%s:%s@%s:%d)! error: %s", gdb_config.user, gdb_config.pwd, gdb_config.host, gdb_config.port, mysql_error(*conn));
		fprintf(stderr, "db_create_conn:connect to database fail(%s:%s@%s:%d)! error: %s", gdb_config.user, gdb_config.pwd, gdb_config.host, gdb_config.port, mysql_error(*conn));
...
//	LOG( LOG_TRACE, "connect to database OK! ");
		fprintf(stderr, "connect to database OK!");
...
//	LOG( LOG_ERROR, "no more db handles to use! ");
	fprintf(stderr, "no more db handles to use! ");
...
//		LOG( LOG_ERROR, "*** db_release_conn ***  index err");
		fprintf(stderr, "*** db_release_conn ***  index err");
		return;
	}	
...
//			LOG( LOG_ERROR, "db_exe: failed,errno:%d,error:%s", mysql_errno(*conn),mysql_error(*conn));
			...
			fprintf(stderr, "db_exe: failed,errno:%d,error:%s", mysql_errno(*conn),mysql_error(*conn));
		return RET_ERR;
    }
...
//  LOG( LOG_TRACE,  "host:%s,user:%s,pwd:%s,dbname:%s,port:%d",gdb_config.host,gdb_config.user,gdb_config.pwd,gdb_config.dbname,gdb_config.port);
	fprintf(stderr,   "host:%s,user:%s,pwd:%s,dbname:%s,port:%d",gdb_config.host,gdb_config.user,gdb_config.pwd,gdb_config.dbname,gdb_config.port);
	...
	//		LOG( LOG_ERROR , "db_create_conn failed![i=%d]",i);
				fprintf(stderr,  "db_create_conn failed![i=%d]",i);
			return RET_ERR;
		}
...
//		LOG ( LOG_TRACE , "connect to database OK![i=%d]",i);
	  fprintf(stderr,  "connect to database OK![i=%d]",i);
	}
...
{	
	...
	*paffect_rows = 0;
	...
	//	LOG ( LOG_ERROR ,"db_exe or mysql_affected_rows failed!");
		fprintf(stderr,  "db_exe or mysql_affected_rows failed!");
...
//			LOG ( LOG_ERROR ,"db_exe or mysql_store_result failed!");
			fprintf(stderr, "db_exe or mysql_store_result failed!");
