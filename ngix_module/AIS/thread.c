...
extern struct redisContext **g_redis_conn;
extern cache_pool_t **g_conn_item_pools;
extern eg_cycle_queue_t **g_conn_item_queues;
extern struct redisContext *g_log_redis_conn;
...
struct event_base *g_reply_handle_base;//add by chenhuizhong
pthread_t g_reply_handle_tid;//add by chenhuizhong
#ifdef MEMCHECK
int g_execute_req = 0;
#endif
...
struct stEV * g_ev=NULL;

static void thread_libevent_process(int fd, short which, void *arg);
static void create_reply_handler(void *arg);
static void *reply_handle_thread(void *arg);
static void reply_handle(int fd, short which, void *arg);
static void reply_handle_logthread(int fd, short which, void *arg);
...
static void create_reply_handler(void *arg)
{
    ...
    if ((ret = pthread_create(&g_reply_handle_tid, &attr, reply_handle_thread, arg)) != 0) {
        fprintf(stderr, "Can't create thread: %s\n",
                strerror(ret));
        exit(1);
    }
...
/* 
 * run handle redis reply loop
 */
static void *reply_handle_thread(void *arg)
{
    int i;
    unsigned long worker_num = (unsigned long)arg + 1;
    
    g_ev = (struct stEV *)malloc(worker_num * sizeof(struct stEV));
    g_reply_handle_base = event_init();

    if (!g_reply_handle_base) {
        fprintf(stderr, "Can't allocate event base\n");
        exit(1);
    }
   ...
   for(i = 0; i < worker_num; ++i)
    {
        if(i < worker_num - 1)
        {
            /*if Edge Trigger is needed, set EV_ET*/
            g_ev[i].idx = i;
            g_ev[i].ev = event_new(g_reply_handle_base, g_redis_conn[i]->fd, EV_READ | EV_PERSIST, reply_handle, g_ev+i);
        }else
        {
            g_ev[i].idx = i;
            g_ev[i].ev = event_new(g_reply_handle_base, g_log_redis_conn->fd, EV_READ | EV_PERSIST, reply_handle_logthread, g_ev+i);
        }

        if (event_add(g_ev[i].ev, 0) == -1) {
            fprintf(stderr, "Can't monitor libevent notify pipe\n");
            exit(1);
        }
    }

    event_base_loop(g_reply_handle_base, 0);
    for(i = 0; i < worker_num; ++i)
    {
        event_free(g_ev[i].ev);    
    }    
    event_base_free(g_reply_handle_base);
    free(g_ev);
    g_ev = NULL;
 ...
 * handle redis reply
 */
void reply_handle(int fd, short which, void *arg)
{
    redisReply* r = NULL;
    struct stEV *EV = (struct stEV *)arg;
    int redis_conn_id = EV->idx;
    struct redisContext *c = g_redis_conn[redis_conn_id];

    if (redisBufferRead(c) == REDIS_ERR) 
    {
        ERROR_LOG(("redisBufferRead(),  c->err[%d][%s], redis_conn_id[%d], socket[%d]\n",
            c->err,
            c->errstr,
            redis_conn_id,
            c->fd));
        if (( REDIS_ERR_EOF==c->err) || (REDIS_ERR_IO==c->err))
	{
     /*
      * when the keepalive connection between AIS and Redis terminated for somewhat reason,
      * we must remove the broken socket fd from epoll, for it's Level Trigger
      */
            int ret= event_del(EV->ev);
            if (0!=ret){
                ERROR_LOG(("event_del() err[%d], redis_conn_id[%d], socket[%d]\n",
                    ret, redis_conn_id, c->fd));
                return;
            }
       }
    ...
    if (redisGetReplyFromReader(c, (void **)&r) == REDIS_ERR) return;

    while(r)
    {
        freeReplyObject(r);
        if (redisGetReplyFromReader(c, (void **)&r) == REDIS_ERR) break;
    }
}

void reply_handle_logthread(int fd, short which, void *arg)
{
    redisReply* r = NULL;
    struct stEV *EV = (struct stEV *)arg;
    int redis_conn_id = EV->idx;
    struct redisContext *c = g_log_redis_conn;
...
static void thread_libevent_process(int fd, short which, void *arg) {
    ...
    item = eg_cq_head(g_conn_item_queues[me->tid]);

    if (NULL != item) {
        conn *c = conn_new(item->sfd, item->init_state, item->event_flags,
                item->read_buffer_size, item->transport, me->base,item->org_start_time, me->tid);
        if (c == NULL) 
        {
        ...
        eg_cq_pop(g_conn_item_queues[me->tid]);
    }
}
...
void dispatch_conn_new(int sfd, enum conn_states init_state, int event_flags,
    ...
    item = (CQ_ITEM *)eg_cq_get_cur_buf(g_conn_item_queues[tid]);
	...
	item->org_start_time = getmocrtime();//for debug time
    eg_cq_push(g_conn_item_queues[tid], item);    
    ...
    if(g_execute_req > 1000)
    {
        ...
        event_base_loopbreak(g_reply_handle_base);
    }
#endif
}
...
void thread_init(int nthreads, struct event_base *main_base) {
    ...
    size = nthreads + 1;
	create_reply_handler((void*)size);
...
void exit_threads()
{
    ...
    pthread_join(g_reply_handle_tid, NULL);
    free(threads);
}
