...
struct redisContext **g_sredis_conn_async;
 ...
 *   }
     command state tokens[]  max_tokens=8
 */
...
void process_command(conn *c, char *command) {
    ...
    uuid_t req_uuid;
    char req_session[1024] = {0};
    uuid_generate(req_uuid);
    bin2str(req_session, req_uuid, sizeof(uuid_t));
  ...
  if (settings.verbose > 1)
    ...
    if (add_msghdr(c) != 0) {
        ...
        cost_time = (t_end.tv_sec - t_start.tv_sec) * 1000 + (t_end.tv_usec - t_start.tv_usec);
	...
	if( c->rbytes >2 && ac->bufr && 
		...
		if(ac->bufr &&ac->bufr[0] == 'a'){
			...
			ais_get(req_session, ac);
            gettimeofday(&end_time, NULL);
            cost_time = (end_time.tv_sec - start_time.tv_sec)* 1000000 + end_time.tv_usec - start_time.tv_usec;
            ERROR_LOG( ("=====>[%s] ais_get total cost_time ->%ld, %lu:%lu", req_session, cost_time, end_time.tv_sec, end_time.tv_usec));
		}
	...
	//		ais_getbody(ac);
		//}
            ...
            gettimeofday(&end_time, NULL);
            cost_time = (end_time.tv_sec - start_time.tv_sec)* 1000000 + end_time.tv_usec - start_time.tv_usec;
            ERROR_LOG( ("=====> ais_wap total cost_time ->%ld, %lu:%lu", cost_time, end_time.tv_sec, end_time.tv_usec));
		}
		else{
			ais_report(ac);
			}
    ...
    {
        gettimeofday(&start_time, NULL);
        ais_imp_clk(req_session, ac);
        gettimeofday(&end_time, NULL);
        cost_time = (end_time.tv_sec - start_time.tv_sec)* 1000000 + end_time.tv_usec - start_time.tv_usec;
        ERROR_LOG( ("=====>[%s] ais_imp_clk total cost_time ->%ld, %lu:%lu", req_session, cost_time, end_time.tv_sec, end_time.tv_usec));
        return;
    }
    ...
    {   
        struct tm date = {0};
        gettimeofday(&start_time, NULL);
        ais_callback(req_session, ac, 0,&date);
        gettimeofday(&end_time, NULL);
        cost_time = (end_time.tv_sec - start_time.tv_sec)* 1000000 + end_time.tv_usec - start_time.tv_usec;
        ERROR_LOG( ("=====>[%s] ais_callback total cost_time ->%ld, %lu:%lu", req_session, cost_time, end_time.tv_sec, end_time.tv_usec));
		return;
		}
...
struct redisContext* get_sredis_conn(int port)
{
    ...
    {
        ERROR_LOG(("g_ctx.config[REDIS_SLAVE_IP] is NULL!!"));
        return NULL;
    }
...
int db_conn_init(int conn_num)
{
    ...
    redis_num = -1;
    char *p_4_strsep_slaveport = g_ctx.config[REDIS_SLAVE_IP_PORT];
    while ((port_arr[++redis_num] = strsep(&p_4_strsep_slaveport, ",")));
    printf("redis_num=%d\n", redis_num);
    ...
    g_sredis_conn_async = (redisContext **)malloc(sizeof(struct redisContext*) * conn_num);
    if (!g_sredis_conn_async) return -1;
    ...
    for( i = 0; i < conn_num; ++i)
    {
        ...
        printf("idx=%d, sredis_port=%d\n", i, atoi(port_arr[index]));
        if(!(g_sredis_conn[i] = get_sredis_conn(atoi(port_arr[index])))) return -1;
        if(!(g_sredis_conn_async[i] = get_sredis_conn(atoi(port_arr[index])))) return -1;
...
void db_conn_destroy(int conn_num)
{
    ...
    for(i = 0; i < conn_num; ++i)
    {
        ...
        redisFree(g_sredis_conn_async[i]);
    ...
    free(g_rredis_conn);
    free(g_sredis_conn);
    free(g_sredis_conn_async);
    free(g_sql_conn);
    g_redis_conn = NULL;
    g_sredis_conn = NULL;
    g_sredis_conn_async = NULL;
    g_sql_conn = NULL;
}
...
int create_conn(cache_pool_t *cp)
{
    ...
    for(i = 0; i < size; ++i)
    {
        ...
        ac->sredisctx_async = g_sredis_conn_async[tid];
